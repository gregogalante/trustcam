...
/**
 * Watermark payload codec v3 — MUST mirror spikes/codec_v3.py bit-for-bit:
 * 256-float message = 128 data bits (captureId, MSB-first)
...
object PayloadCodecV3 {
    ...
    /** 256-float message (0/1) for the embedder graph; captureId is 16 bytes. */
    fun encode(captureId: ByteArray): FloatArray {
        require(captureId.size == ID_BYTES) { "captureId must be $ID_BYTES bytes" }
        require(captureId.any { it != 0.toByte() }) { "captureId must not be all-zero" }
        val msg = FloatArray(BITS)
        var hi = 0L
        var lo = 0L
        for (i in 0 until DATA_BITS) {
            // data bits MSB-first straight from the id bytes
            val bit = (captureId[i ushr 3].toInt() shr (7 - (i and 7))) and 1
            if (bit == 1) {
                msg[i] = 1f
                hi = hi xor rowHi[i]
                lo = lo xor rowLo[i]

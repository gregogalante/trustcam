// Payload codec v2 for the 256-bit VideoSeal message — JS port of the decoder
// half of spikes/codec_v2.py (bit-identical wire format).
// Layout: 128 data bits (proofId 24 | pHash 104, MSB-first) + 124 BCH parity
// + 4 zero pad. Code: BCH(255,131,t=18) over GF(2^8), primitive poly 0x11d
// (Linux-kernel bch, as produced by python bchlib), shortened to 252 coded
// bits — the 3 missing leading data bits are implicit zeros.
...
;(typeof window !== 'undefined' ? window : globalThis).TrustCamCodecV2 = (() => {
  ...
  // Returns { proofId, phashHex, corrected } or null when uncorrectable
  // (or the corrected proofId is 0).
  function decode (softBits) {
    ...
    let proofId = 0
    for (let i = 0; i < 24; i++) proofId = proofId * 2 + hard[i]
    if (proofId === 0) return null

    let phashHex = ''
    for (let i = 24; i < DATA_BITS; i += 4) {
      const nib = (hard[i] << 3) | (hard[i + 1] << 2) | (hard[i + 2] << 1) | hard[i + 3]
      phashHex += nib.toString(16)
    }
    return { proofId, phashHex, corrected }
  }
